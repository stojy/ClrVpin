...
namespace ClrVpin.Shared.Utils
{
    public static class FileUtils
    {
        ...
        public static FileDetail Rename(Hit hit, LocalGame localGame, ICollection<HitTypeEnum> supportedHitTypes, IEnumerable<string> kindredExtensions)
        {
            var renamed = false;
            ...
            if (supportedHitTypes.Contains(hit.Type))
            {
                ...
                var newFile = GetCorrectFile(localGame, _settings.GetContentType(hit.ContentTypeEnum).Category, hit.Directory, hit.Extension);
        ...
        public static string GetCorrectFile(LocalGame localGame, ContentTypeCategoryEnum category, string path, string extension)
        {
            // determine the correct name - which has a different calculation for media vs pinball :(
            var correctContentName = Content.GetName(localGame, category);
